    ...
    return (
        <>
            <section className="relative">
                ...
                <div className={`absolute z-20 bottom-[10px] md:bottom-[40px] left-2/4 flex -translate-x-2/4 gap-[10px] [&>*]:text-[1rem] text-MO1`}>
                ...
                >
                    {
                        data.map((e, i) => {
                            return (
                                <div className={`relative w-full h-[500px] max-h-[850px] overflow-hidden`} key={ i }>
                                    <div className={`z-50  md:from-white/0 md:to-white/0 absolute max-w-screen-2xl flex flex-col justify-end md:justify-center left-2/4 pb-[75px] pt-[150px] md:py-0 bottom-0 md:bottom-[80px] w-full -translate-x-2/4 md:-translate-y-2/4 px-[20px]`}>
                                        {
                                            e.minitext != '' && (
                                                <p className={`uppercase text-end md:text-center text-[20px] md:text-[36px]`}>{ e.minitext }</p>
                                            )
                                        }
                                        <ElementCond el={ i == 0 ? 'h1' : i == 1 ? 'h2' : 'h3' } className={`text-[36px] md:justify-center md:text-[36px] uppercase flex flex-col md:flex-row justify-end items-end md:gap-[15px] leading-[100%]`}>
                                            {
                                                e.text.split('\n').map((m, i) => (
                                                    <span key={ i } className={`flex gap-[0.5rem] md:gap-[1rem]`}>
                                                        {
                                                            m.split(' ').map((b, i) => (
                                    ...
                                    <div className={``}>
                                        ...
                                        <Image className={`object-cover w-full h-[100%] -z-30`} src={ typeof(e.desktop) != 'string' ? URL.createObjectURL(e.desktop) : process.env.NEXT_PUBLIC_MEDIA_DOMAIN + e.desktop } layout='fill' alt={`Kain Moeji Home Banner ${ i }`}/>
                                    </div>
